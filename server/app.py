...
def animal_by_id(id):
    ...
    response_body += f'<ul>Species: {animal.species}</li>'
...
def zookeeper_by_id(id):
    ...
    respnse_body += f'<ul> Name:{zookeeper.name}</ul>'
    response__body += f'<ul>Birthday:{zookeeper.birthday}<ul>'

    for animal in zookeeper.animals:
        response_body += f'<ul>{animal.name}</ul>'

    return make_response(resonse_body)
